...
#     "TATACONSUM.NS", "TATAMOTORS.NS", "TATAPOWER.NS", "TATASTEEL.NS", "TCS.NS", "TECHM.NS",
#     "TITAN.NS", "TORNTPHARM.NS", "TORNTPOWER.NS", "TRENT.NS", "TVSMOTOR.NS", "UBL.NS", "ULTRACEMCO.NS",
#     "UNIONBANK.NS", "UPL.NS", "VEDL.NS", "VOLTAS.NS", "WHIRLPOOL.NS", "WIPRO.NS", "ZEEL.NS"  # Truncated sample
# ]
...
#         open_price = p915
#         prev_close = extract_price(previous_day, '15:29:00')
#         current_price_time = now.strftime("%H:%M:00")
#         current_price = extract_price(latest_day, current_price_time)

#         # Additional metrics
#         prev_day_high = round(daily.loc[str(previous_day)]['High'], 2) if str(previous_day) in daily.index else None
#         prev_day_low = round(daily.loc[str(previous_day)]['Low'], 2) if str(previous_day) in daily.index else None
#         week_high = round(daily['High'].max(), 2)
#         week_low = round(daily['Low'].min(), 2)
